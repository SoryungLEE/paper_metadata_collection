...
from libs import real3_ror_mapping_processor_patched as real3
...
def _parse_real3_ror_counts_from_log(log_path: Path):
    """
    run_log_{year}.txt 에서
      - [real3] ROR URL이 전혀 없는 행 수(before): N
      - [real3] ROR 보강 완료: +M건
    을 파싱해서 (N, M) 반환. 실패 시 (None, None)
    """
    import re
    try:
    ...
    m_before = re.search(r"\[real3\]\s*ROR URL이 전혀 없는 행 수\(before\):\s*(\d+)", text)
    m_plus   = re.search(r"\[real3\]\s*ROR 보강 완료:\s*\+(\d+)건", text)

    before = int(m_before.group(1)) if m_before else None
...
def _update_metrics_ror_for_piece(
    metrics_path: Path,
    final_csv_path: Path,
    log_file: Path | None = None
) -> None:
    """
    ✅ ROR 지표는 run_log_{year}.txt(=real3 stdout) 기준으로 저장한다.
      - ror_missing_before_extract = [real3] ... (before) 값
      - ror_enriched              = [real3] ROR 보강 완료: +X건
      - ror_missing               = before - enriched
    """
    import pandas as pd
    import csv as _csv
    ...
    if log_file is not None and Path(log_file).exists():
        before, plus = _parse_real3_ror_counts_from_log(Path(log_file))

    # 2) 로그 파싱 실패 시: 아무 것도 안 쓰고 종료(원하면 여기서 폴백 계산 넣어도 됨)
    if before is None or plus is None:
        print(f"[WARN] ROR metrics skipped: cannot parse log_file={log_file}")
        return

    missing_after = max(0, int(before) - int(plus))
    enriched = max(0, int(plus))
    ...
    # ✅ ROR 관련 키는 “로그값”으로만 덮어쓴다
    kv["ror_missing_before_extract"] = str(int(before))
    kv["ror_enriched"] = str(int(enriched))
    kv["ror_missing"] = str(int(missing_after))

    # 요구사항: metrics.csv에서 ror_missing_after_extract는 제거
...
def _run_one_piece(issn: str, year: int, email: str,
    ...
    _update_metrics_ror_for_piece(metrics_path, local_out, log_file=log_file)
...
def _collect_merge(issns: List[str], year_start: int, year_end: int) -> Path:
    ...
    for p in piece_paths:
        ...
        # 2) C 엔진 실패 시: python 엔진으로 재시도
        #    → 문제 있는 행이 있어도 전체 파이프라인이 죽지 않도록 on_bad_lines='skip'
        try:
            df_piece = pd.read_csv(
                p,
                engine="python",
                ...
                quotechar='"',
                escapechar='\\',
                on_bad_lines='error',   # 🔴 'error' → 'skip' 으로 완화했다가 error로 다시 돌아옴
            )
            dfs.append(df_piece)
            continue
        except Exception as e:
