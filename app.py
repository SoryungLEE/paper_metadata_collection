...
def _summary_top_nodes_and_edges(final_csv_path: str):
    ...
    for c in candidates:
        ...
        scores = tmp[c].head(10).tolist()

        # 노드 이름
        nodes_cols[f"{c}: 노드"] = names + [""] * (10 - len(names))
        # 중심성 점수(소수 네 자리 문자열로 포맷)
        nodes_cols[f"{c}: 점수"] = [f"{v:.4f}" for v in scores] + [""] * (10 - len(scores))
    ...
    # 상위 10개 엣지 + 동시출현 수(= score)
    top_pairs = edge_counter.most_common(10)
    top_edges = [k for k, _ in top_pairs]
    top_counts = [v for _, v in top_pairs]

    # 기존 UI 유지: 각 centrality별 “...: 엣지” 열은 그대로 두고,
    # 오른쪽에 동시출현 수(점수) 열을 하나 추가
    edges_cols = {
        **{f"{c}: 엣지": top_edges + [""] * (10 - len(top_edges)) for c in candidates},
        "동시출현 수": top_counts + [0] * (10 - len(top_counts)),
    }
    df_edges = pd.DataFrame(edges_cols, index=[f"Top {i}" for i in range(1, 11)])

    # candidates가 2개/1개인 경우도 열 정렬 보정
    # 노드: 각 centrality별 "노드 → 점수" 순서로 정렬
    node_cols = []
    for m in ["degree", "eigenvector", "betweenness", "closeness"]:
        if f"{m}: 노드" in df_nodes.columns:
            node_cols.append(f"{m}: 노드")
        if f"{m}: 점수" in df_nodes.columns:
            node_cols.append(f"{m}: 점수")
    if node_cols:
        df_nodes = df_nodes[node_cols]

    # 엣지: 기존 정렬 유지 + 동시출현 수 맨 뒤
    edge_cols = [c for c in [f"{m}: 엣지" for m in ["degree","eigenvector","betweenness","closeness"]]
                 if c in df_edges.columns]
    if "동시출현 수" in df_edges.columns:
        edge_cols.append("동시출현 수")
    if edge_cols:
        df_edges = df_edges[edge_cols]
